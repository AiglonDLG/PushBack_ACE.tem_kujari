...
diag_log "/**************************************** COS INIT START ****************************************\";

if (isnil "SERVER") then {Hint "You must ADD a object named SERVER";Player Sidechat "You must ADD a object named SERVER";}
else {
	if (isServer) then {
		IF (!isnil ("COScomplete")) then {
			Hint "Check your call. COS was called twice!";
		}
		else{
			...
			COSmotPool =["C_Hatchback_01_F","C_Hatchback_01_sport_F","C_Offroad_01_F","C_SUV_01_F","C_Van_01_box_F","C_Van_01_fuel_F"];
...

			{
				...
				if (({_name==_x} count blackListTowns)>0 OR (_name == "")) then {} 
				else
				{
