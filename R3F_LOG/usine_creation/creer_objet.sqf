					...
					if !(isNull _objet) then
					{
						...
						else
						{
							...
							else
							{
								...
								(format ["USINE %5: %1%2%3%4", getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName"), " pour un cout de ", _cout, " credits.", name player]) remoteExec ["diag_log", 2, false]; 
							};
							
							clearItemCargoGlobal _objet; 
							clearMagazineCargoGlobal _objet; 
							clearWeaponCargoGlobal _objet;
							clearBackpackCargo _objet;
							_objet addItemCargoGlobal ["ToolKit", 1];
							if (typeOf _objet == "BwS_VBCI") then
							{
								_objet animate ["showSLATHull", 1];
								_objet animate ["showSLATTurret", 1];
							};
